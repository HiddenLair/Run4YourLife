...
Shader "Custom/Dissolve" {
    Properties {
        ...
        _Ramp ("Ramp", 2D) = "black" {}
        [MaterialToggle] _UseEmision ("UseEmision", Float ) = 0
        _Emission ("Emission", 2D) = "white" {}
        [HDR]_EmisionColor ("EmisionColor", Color) = (1,1,1,1)
        _EmissionForce ("EmissionForce", Float ) = 1
...
////// Emissive:
                ...
                float3 emissive = ((_EmisionColor.rgb*_Emission_var.rgb*pow((1.0 - saturate(dot(normalize(viewDirection),i.normalDir))),(-1*_EmissionForce))*_UseEmision)+(node_2357.rgb*UNITY_LIGHTMODEL_AMBIENT.rgb));
/// Final Color:
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float2 node_534 = float2((1.0 - saturate((node_4786*8.0+-4.0))),0.0);
                float4 node_2357 = tex2D(_Ramp,TRANSFORM_TEX(node_534, _Ramp));
                o.Emission = ((_EmisionColor.rgb*_Emission_var.rgb*pow((1.0 - saturate(dot(normalize(viewDirection),i.normalDir))),(-1*_EmissionForce))*_UseEmision)+(node_2357.rgb*UNITY_LIGHTMODEL_AMBIENT.rgb));
