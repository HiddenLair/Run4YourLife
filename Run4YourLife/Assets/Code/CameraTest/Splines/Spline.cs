...
public class Spline : MonoBehaviour {
    ...
    {
        if (knots == null)
            throw new System.ArgumentNullException("knots");
        int n = knots.Length - 1;
        if (n < 1)
            throw new System.ArgumentException("At least two knot points required", "knots");
        if (n == 1)
        ...
        {
            // First control point
            firstControlPoints[i] = new Vector3((float)x[i], (float)y[i]);
            // Second control point
            if (i < n - 1)
                secondControlPoints[i] = new Vector3((float)(2 * knots[i + 1].x - x[i + 1]),(float)( 2 * knots[i + 1].y - y[i + 1]));
            else
                secondControlPoints[i] = new Vector3((float)(knots[n].x + x[n - 1]) / 2,(float)(knots[n].y + y[n - 1]) / 2);
        }
    }
